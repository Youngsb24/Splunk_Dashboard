...
<h2>Description</h2>
In this project I will be showing ways to maneuver through splunk. provided a detailed step-by step example on how to create multiple visualizations for a dashboard. As well as include how to edit the dashboard properly to make it look appealing. The dashboard I will be creating is based upon Airbnb's hosted in New York City. I will create a 6-panel dashboard to break down different views of selecting an Airbnb in New York. Lastly, i'll be using a standalone splunk instance
...
<h3>Step 1</h3>
You want to create an Index. This is were the file downloaded will be stored. Go to settings --> then click indexes --> new indexes . Since the file i'm using is dealing with Airbnbs , my indexes title will be labled "Airbnb" 
...
<h5>Step 3</h5>
Now we want to search and make sure our file uploaded succesfully, so we click on the " Search and Report" and click search, type in "index=airbnb" remember the field names are case sensitive and field values are not. After changing the time range picker to "all time" and "verbose mode" then run the search and you should see this.
...
As you can see I break down the top 5 neighborhoods in New York. I included the top 10 zip codes around those cities, and the prices of airbnb rooms per night dependent on how long a customer is planning on staying. And included the property type of airbnb from houses; regular apartments , also included lofts and townhouses. Lastly, I just included other panels because I was bored :).
